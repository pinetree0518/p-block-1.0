//test는 EntSave,StrongBlock, SpecialBlock, ExpressBlock을 참고해서 만들었습니다.
...
const LibraryCreator = {    //필수
    start: (blocksJSON, category, text) => {
        ...
        if (typeof useWebGL == "undefined") {
            ...
            // 아이콘 적용
            $('head').append(`<style>#entryCategory${category}{background-image:url(기본svg);background-repeat:no-repeat;margin-bottom:1px}.entrySelectedCategory#entryCategory${category}{background-image:url(반전svg);background-color:#00FF98; color:#000000}</style>`)  //블록 이미지 & 블록 꾸러미 색 설정
            // 카테고리 이름 적용
...
let blockPOST
alert('p-block 1.0 설치를 시작합니다.')
document.title = "test"; //블록꾸러미 이름 성정.
LibraryCreator.start(blocks, 'API', 'test')  //블록설치. 블록꾸러미 이름 설정.
alert("p-block 1.0 설치가 완료되었습니다.")
